...
def open_positions(client):
    ...
    for index, row in df.iterrows():
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            # Establish if potintial trade
            if abs(z_score) >= ZSCORE_THRESH:
                # Ensure like-to-like not to already open (diversify trading):
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    # Get acceptible price in string format with correct number of decimals
                    ...
                    failsafe_base_price = float(
                        base_price) * 0.05 if z_score < 0 else float(base_price) * 1.70
